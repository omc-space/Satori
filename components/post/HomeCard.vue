...
<template>
  <div class="relative h-[100px] max-w-120 min-w-0 flex-1 overflow-hidden border border-gray/20 rounded-md">
    <NuxtLink :to="`/post/${post.id}`" class="group relative inline-block w-full">
      ...
      <div class="absolute bottom-0 left-0 top-0 z--1 h-full w-full rounded-md bg-gray-100 dark:bg-white/30">
        <CommonLazyLoadImage v-if="post.images.length" class="h-full w-full rounded-md bg-transparent" :src="post.images[0].src" />
      </div>
    </NuxtLink>
  </div>
</template>
